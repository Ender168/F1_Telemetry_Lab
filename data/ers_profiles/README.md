...
Начиная с v0.10.2 профиль схемы 2 добавляет к `NEUTRAL`, `ATTACK` и `DEFEND` интенсивность `PRESSURE` / `CRITICAL`, скорость изменения разрыва, SOC-коридор по контрольным точкам, DRS-контекст и остаток кругов.

`China_Race.json` является первым продвинутым эталоном схемы 2. Он имеет `profile_id: china-race-advanced-v2` и `profile_revision: 3`. Штатный China-профиль предыдущих версий обновляется один раз, а его копия сохраняется рядом как `China_Race.json.pre-0.10.2.bak`. Профиль с другим пользовательским `profile_id` не перезаписывается.
