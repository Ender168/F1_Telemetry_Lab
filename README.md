...
Версия 0.9.1 исправляет Live-ввод прототипа ERS: F7/F8 отправляются аппаратными scan-кодами с удержанием 80 мс, а UDP-подтверждение засчитывается только после фактически наблюдаемого следующего режима. Для каждой трассы используется отдельный редактируемый JSON-профиль без пересборки приложения. Первый профиль для гонки в Китае построен по выводам разбора R03: Medium является базой, None используется для контролируемого восстановления, основной Boost расположен после T13, а короткие дополнительные включения разрешаются только в бою или при высоком заряде.

Вся цепочка 0.8.x сохранена: полный `session.sqlite` остаётся восстанавливаемым источником телеметрии, после анализа применяется lean storage profile, а analysis pack содержит согласованные full и compact снимки.
...
- Хранение необходимых исходных UDP-пакетов для повторного анализа без повторной гонки.
...
- Полный согласованный `session.sqlite` и компактный `chatgpt_pack.sqlite` в analysis ZIP.
- ERS Autopilot с режимами Off, Dry-run и Live, отдельными профилями трасс и журналом решений.
- Feedback-контроль стандартного ERS по фактическому `ersDeployMode`, один подтверждаемый шаг на одно нажатие.
- Жёсткие блокировки Live-ввода в онлайне и небезопасных состояниях гонки.
...
| UDP Port | 20777 |
| UDP Send Rate | 60 Hz |
| ERS Assist | Off для режима Live |
| Increase / Decrease ERS Deploy Mode | F8 / F7 для режима Live |
...
Перед стартом записи выберите режим в строке `Live`:

| Режим | Поведение |
|---|---|
| `Off` | Контроллер не запускается |
| `Dry-run` | Решения рассчитываются и записываются, клавиши не отправляются |
| `Live` | Решения выполняются удерживаемыми scan-code нажатиями F7/F8 с подтверждением каждого следующего режима по UDP |

Начинайте с `Dry-run`. После сессии проверьте `ers_control_log.csv`, и только затем переходите к `Live`.

Профили находятся в `<root>/ers_profiles/`. Встроенные файлы копируются туда только при отсутствии и никогда не перезаписывают пользовательские правки. Новый алгоритм гонки добавляется отдельным JSON-файлом и подбирается по `track_id` и `session_types`. Кнопка `Open ERS profiles` открывает каталог вместе с инструкцией по созданию следующего профиля.

Live-ввод разрешён только для офлайн-сессии, сухого профиля, активного гоночного круга, свежей телеметрии и активного окна F1 25. Он блокируется при включённом игровом ERS Assist, паузе, режиме зрителя, Safety Car, VSC, formation lap и нахождении в питах. F12 аварийно отключает ввод до следующей записи.

Прототип управляет только стандартным `ersDeployMode`: None, Medium, Hotlap и Boost. Overtake Mode из Packet 16 намеренно остаётся под ручным управлением.
...
ers_profile_used.json       точный снимок выбранного алгоритма ERS
ers_control_log.csv         решения, блокировки, команды и UDP-подтверждения ERS
<session-name>.zip          пакет для передачи или архивирования
```

Analysis ZIP включает согласованный снимок `session.sqlite`, компактную базу и все sidecar-файлы ERS. Локальная рабочая база остаётся в папке сессии.

Подробности: [архитектура](docs/ARCHITECTURE.md), [схема и правила данных](docs/DATA_MODEL.md), [release notes](docs/RELEASE_0.9.1.md), [история изменений](CHANGELOG.md).
...
- ERS Live не считается автономной системой безопасности и должен сначала проверяться в Dry-run и короткой офлайн-сессии.
