...
Версия 0.9.0 добавляет прототип автоматического управления стандартными режимами ERS. Для каждой трассы используется отдельный редактируемый JSON-профиль без пересборки приложения. Первый профиль для гонки в Китае построен по выводам разбора R03: Medium является базой, None используется для контролируемого восстановления, основной Boost расположен после T13, а короткие дополнительные включения разрешаются только в бою или при высоком заряде.
...
| `Live` | Решения выполняются через F7/F8 с подтверждением нового режима по UDP |
...
Подробности: [архитектура](docs/ARCHITECTURE.md), [схема и правила данных](docs/DATA_MODEL.md), [release notes](docs/RELEASE_0.9.0.md), [история изменений](CHANGELOG.md).
